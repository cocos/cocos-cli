...
Prefab 是 scene 模块里最容易出现时序问题的区域。它同时修改两类持久化数据：

- 场景里的 prefab instance 结构和 `_prefab` metadata。
- prefab asset 文件内容。

这两类数据之间还存在异步刷新链路：prefab asset 保存后，asset system 会发送 `asset-change`，`PrefabService.onAssetChanged()` 再触发 soft reload 当前 editor。soft reload 是编辑器状态刷新，不是 dirty 判定来源。

dirty 的权威来源仍然是 undo stack 是否离开保存点，即 `Undo.isDirty()`。`node:change` / `component:*` 只用于 hierarchy、inspector、扩展事件刷新，不能作为 dirty 判定依据。
...
从数据流看，真正的问题不是 `revertToPrefab` 自己逻辑错误，而是前一次 `applyPrefabChanges` 保存 prefab asset 后排队的 soft reload 可能晚到：

1. `applyPrefabChanges` 保存 prefab asset。
2. asset system 发送 `asset-change`。
3. `PrefabService.onAssetChanged()` 把当前 editor reload 放进 500ms debounce。
...
所以问题本质是 prefab mutation 与 prefab soft reload 缺少统一编排，导致上一轮 reload 能打断下一轮 mutation。

## 当前方案

当前修复不是新增固定 sleep，也不是扩大 `Editor.lock()`。

做法是让 `PrefabSoftReloadScheduler` 显式维护 reload 状态：

- pending timer。
- 正在执行的 flush promise。
- 等待指定 asset reload 完成的 waiters。
- 等待 scheduler idle 的 waiters。

Prefab 可变 API 执行前先等待本服务已排队的 soft reload 进入 idle：
...
`applyPrefabChanges` 还会在当前 prefab asset 确实影响当前打开 editor 时，等待对应 asset reload 完成后再 capture after snapshot 并 push `PrefabApplyCommand`。

这样保证 API 返回时，调用方不会马上读到半刷新状态，也避免上一轮 reload 晚到打断下一轮 mutation。

## 为什么不用 `Editor.lock()`

曾经验证过把 prefab 可变 API 包在 `Editor.lock()` 里，但这会引入新的互等风险：

- prefab mutation 内部会触发 asset save / load。
- asset change 会触发 editor reload。
- editor reload 会等待 editor lock。
- mutation 又可能等待 reload 或 asset ready。

结果是 prefab dirty/undo contract 测试从正常十几秒变成 100 秒超时。

因此当前方案只等待 `PrefabSoftReloadScheduler` 自己排队的 reload idle，不持有全局 editor lock。

## 行为变化

当前改动会收敛 prefab API 的异步返回时机：
...
这可能让连续 prefab 操作比旧行为多等一次 debounce + reload，但换来的是稳定的场景状态。
...
当前方案修的是已知根因：prefab soft reload 与 prefab mutation 交错执行。
...
- mutation 前后 snapshot。
- prefab asset save/load。
- soft reload preserve / wait / cancel。
- undo command push。
- dirty 状态翻转。
- asset-change 副作用隔离。

重构目标不是改变 prefab 业务语义，而是把“场景结构变更、asset 内容变更、reload、undo/dirty”放进同一条可验证的编排链路。
