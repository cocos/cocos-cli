...
export class EngineCompiler {
    ...
    async generateCompiler(options?: { isNative?: boolean, isWebview?: boolean }): Promise<QuickCompiler> {
        ...
        if (options?.isNative) {
            ...
            const editorFeatures = await this.filterEngineModules(env, allFeatures);
            // The standalone scene Worker uses a Node-owned offscreen WebGL context for baking.
            // Keep EmptyDevice for machines running data-only operations without graphics support.
            if (!options?.isWebview && !editorFeatures.includes('gfx-webgl')) {
                editorFeatures.push('gfx-webgl');
            }
            this.editorFeaturesCache.push(...editorFeatures);
