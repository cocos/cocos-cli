...
import { existsSync, statSync, readJSONSync, writeJSONSync, readFileSync, removeSync, copy, readJsonSync, remove } from 'fs-extra';
...
describe('测试 db 的操作接口', function() {
    ...
    describe('delete-asset', () => {
        describe('删除文件夹', function() {
            ...
            it('删除文件夹后源文件不存在', async () => {
                await assetManager.removeAsset(`${testInfo.testRootUrl}/${name}.directory3`);
        ...
        it('使用 url 删除普通资源', async function() {
            await assetManager.removeAsset(`${testInfo.testRootUrl}/${name}a.normal3`);
    ...
    describe('save-asset', () => {
        it('保存普通资源', async function() {
            await assetManager.saveAsset(`${testInfo.testRootUrl}/${name}.normal`, 'test2');
    ...
    describe('reimport-asset', () => {
        ...
        it('普通资源 uuid 的 reimport', async () => {
            const uuid = await assetManager.queryUUID(`${testInfo.testRootUrl}/${name}.normal`);
    ...
    describe('new-asset', () => {
        ...
        it('创建普通资源 overwrite', async function() {
            ...
            test.each([
                'UUID 需要与指定的一致', (asset: IAssetInfo) => expect(asset && asset.uuid).toStrictEqual(uuid),
                '指定 userData 生效', () => {
