...
export async function generateOptions(options: IOhosInternalBuildOptions) {
    ...
    // const ohosSDKInfo = await Editor.Message.request('program', 'query-program-info', 'ohosSDK');
    // const ohosNDKInfo = await Editor.Message.request('program', 'query-program-info', 'ohosNDK');
    if(!ohos.sdkPath) {
