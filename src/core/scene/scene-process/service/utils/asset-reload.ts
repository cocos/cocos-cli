...
let bundleRefresher: ((serverURL?: string) => Promise<unknown>) | undefined;

/** Optional browser integration; the Node worker resolves imported assets directly. */
export function setAssetBundleRefresher(refresher: typeof bundleRefresher): void {
    bundleRefresher = refresher;
}

export async function refreshAssetBundles(serverURL?: string): Promise<void> {
    await bundleRefresher?.(serverURL);
}
