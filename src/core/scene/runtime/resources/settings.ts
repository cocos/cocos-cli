import type { IPreviewSettingsResult } from '../../../builder/@types/private/index';
...
async function generatePreviewSettings(startScene: string, sceneEditor: boolean): Promise<IPreviewSettingsResult> {
    const { assetManager } = await import('../../../assets/index');
    let result: IPreviewSettingsResult;
    try {
        const { getPreviewSettings, queryDefaultBuildConfigByPlatform } = await import('../../../builder/index');
