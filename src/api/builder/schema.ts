...
export const SchemaWebMobilePackages = z.object({
    ...
    embedWebDebugger: z.boolean().default(false).describe('Whether to embed Web debugging tools'), // 是否嵌入 Web 端调试工具
...
const BuildConfigCoreFields = z.object({
    ...
    md5Cache: z.boolean().describe('Add MD5 information to all built asset filenames to solve CDN asset caching issues'), // 给构建后的所有资源文件名将加上 MD5 信息，解决 CDN 资源缓存问题
    ...
    server: z.string().describe('Remote asset server address'), // 远程资源服务器地址
    startSceneAssetBundle: z.boolean().describe('Specify the initial scene as a remote Bundle package'), // 指定初始场景为远程 Bundle 包
    bundleConfigs: z.array(SchemaBundleConfig).describe('Specify parameters for building Bundles, if not passed, package according to the original configuration of all Bundles in the project'), // 构建 Bundle 的指定包含传参，未传递时按照项目内所有 Bundle 的原始配置打包
...
export const SchemaBuildRuntimeOptions = z.object({
    configPath: z.string().optional().describe('Build configuration JSON file address'), // 构建配置 JSON 文件地址
    ...
    // logDest: z.string().optional().describe('Specify build log output address' // 指定构建日志输出地址
    // ),
});
...
export const SchemaPreviewSettingsResult = z.object({
    ...
    bundleConfigs: z.array(z.object({
        name: z.string().describe('Bundle name'), // bundle 名称
        uuids: z.array(z.string()).describe('List of asset UUIDs in the bundle'), // bundle 中的资源 UUID 列表
        paths: z.record(z.string(), z.array(z.string())).describe('List of asset paths in the bundle'), // bundle 中的资源路径列表
        ...
        versions: z.record(z.string(), z.array(z.union([z.string(), z.number()]))).describe('List of asset versions in the bundle'), // bundle 中的资源版本列表
        redirect: z.array(z.union([z.string(), z.number()])).describe('List of redirected assets in the bundle'), // bundle 中的重定向资源列表
        debug: z.boolean().describe('Whether the bundle is in debug mode'), // bundle 是否是 debug 模式
        types: z.array(z.string()).optional().describe('List of asset types in the bundle'), // bundle 中的资源类型列表
        encrypted: z.boolean().optional().describe('Whether assets in the bundle are encrypted'), // bundle 中的资源是否加密
        ...
        extensionMap: z.record(z.string(), z.array(z.union([z.string(), z.number()]))).describe('List of extension assets in the bundle'), // bundle 中的扩展资源列表
