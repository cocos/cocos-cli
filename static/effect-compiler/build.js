...
if (options.engineRoot) {
    if (options.engineRoot === 'default') {
        options.engineRoot = defaultEngineRoot;
    }
    if (fs.existsSync(options.engineRoot)) {
...
const { default: preload } = (Module.createRequire || Module.createRequireFromPath)(ps.resolve(editorRoot, 'app/index.js'))('cc/preload');
(async () => {
    await preload({
...
function main() {
    const shdcLib = require(ps.join(editorRoot, 'app/modules/engine-extensions/extensions/engine-extends/static/effect-compiler'));
    ...
    const debugDir = ps.join(editorRoot, 'effects.ts');
